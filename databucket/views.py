...
def home(request):
    n_dps = FirestormReading.objects.all()
    print len(n_dps)
    data_points="["
    for n in n_dps:
...
def latest(request, amount):
    ...
    if amount >0 and total <=amount:
        observationList = ObservedDevice.objects.all()[:amount]
    else:
        observationList = ObservedDevice.objects.all()
    return render_to_response('observation_list.html',{'observationList':observationList, 'total':total},
...
def firestorm(request,  amount):
    ...
    if amount == 'all':
        ...
        charts_firestorm=[]
        data = FirestormReading.objects.filter(firestorm=fire).order_by('recorded')[:display_points]
        print data
